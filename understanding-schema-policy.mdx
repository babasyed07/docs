...
- **Discretionary Access Control**: Scoped to specific principals.
- **Membership-Based (RBAC)**: e.g., `principal in Group::...`
- **Relationship-Based (ReBAC)**: e.g., when `{ principal in resource.owners }`
- **ABAC**: Using **when/unless** with principal or resource attributes.
...
3. **Hierarchy and Relationships**  
   Schemas can model relationships and hierarchies, such as:
   - **Nested albums** (Album in Album)
   - **Users in groups** (principal in Group::"engineering")
   **Used with**:
   - **Scope**: `resource in Album::"alice_vacation"`
   - **Conditions**: `principal == resource.owner`
