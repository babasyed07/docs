...
7. **Publish Policy**
   ...
   <Info>
    Before you can publish policies, you must first configure **Policy Publish settings** in the **Policy Store** This includes selecting the policy type (AVP, Cedar, or OPA) and setting up the connection details such as region, external ID, and account number. Without this configuration, publishing will not be possible.   
    **Example**: In the SecureBank store, you would set Policy Type to **AVP**, provide the AWS Account ID <code>123456789012</code>, and select Region <code>us-east-2</code> before publishing policies.
   </Info>
