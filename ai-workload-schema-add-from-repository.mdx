...
When you create an agentic policy store, **Add from Repository** lets you pull in agents that are already in your **Reva inventory** — the tenant-wide **`GlobalAgentWorkloadApp`** store where every ingested entity lands. No template upload needed for this store.

Your inventory is populated by any of these:

- **Entity ingestion API** — push agents, tools, models, and relationships programmatically. See [data ingestion](/ai-workload-developer-settings#ingest-entities-via-the-api) and [Create Entities](/api-reference/entity-ingestion/create-entities).
